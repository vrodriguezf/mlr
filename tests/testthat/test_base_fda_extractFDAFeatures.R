...
test_that("extractFDAFeatures", {
  methods = list("UVVIS" = extractFDAFourier(), "NIR" = extractFDAFourier())
  ext = extractFDAFeatures(fuelsubset.task, feat.methods = methods)
  # check output data
  df = getTaskData(ext$task)
  assert_data_frame(df, nrows = 129L, ncol = 367L)
  # Assert target and scalars are returned
  expect_subset(c("heatan", "h20"), colnames(df))
  expect_character(setdiff(colnames(df), c("heatan", "h20")), pattern = "phase")
})

test_that("Single functional can be extracted multiple times", {
  ...
  # check output data
  df = getTaskData(ext$task)
  expect_data_frame(df, nrow = 129L, ncol = 154L)
  expect_character(colnames(df), pattern = "(phase)|(multires)|(h20)|(heatan)")
  ...
  # check output data
  df = getTaskData(ext$task)
  expect_data_frame(df, nrow = 129L, ncol = 385L)
  expect_character(colnames(df),
                   pattern = "(NIR.phase)|(VIS.phase)|(NIR.multires)|(VIS.multires)|(h20)|(heatan)")
})

test_that("Wrong methods yield informative errors", {
...
test_that("Wavelet method is equal to package", {
  ...
  # extract and reextract Wavelets
  gp.subset = subsetTask(gunpoint.task, features = 1L)
...
test_that("getFDAMultiResFeatures works on data.frame", {
  ...
  # extract and reextract MultiRes
...
test_that("Fourier is equal to package", {
