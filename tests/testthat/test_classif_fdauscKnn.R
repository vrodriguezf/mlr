context("Rlearner_classif_fdauscknn")

test_that("classif_fdauscknn behaves like original api", {
  ...
  lrn = makeLearner("classif.fdauscknn", par.vals = list(knn = 1L, trim = 0.5))
...
test_that("predicttype prob for fda.usc", {
  requirePackagesOrSkip("fda.usc", default.method = "load")
  lrn = makeLearner("classif.fdauscknn", par.vals = list(knn = 1L, trim = 0.5), predict.type = "prob")
...
test_that("resampling fdauscknn", {
  requirePackagesOrSkip("fda.usc", default.method = "load")
  lrn = makeLearner("classif.fdauscknn", par.vals = list(knn = 1L, trim = 0.5), predict.type = "prob")
