...
test_that("tuheThreshold works with all tuning methods", {
  ...
  mbo.ctrl = mlrMBO::setMBOControlTermination(mbo.ctrl, iters = 1)
  mbo.lrn = makeLearner("regr.lm", predict.type = "se")
  mbo.des = generateDesign(n = 4, par.set = ps)
  ctrls = list(
    ...
    design = makeTuneControlDesign(design = mbo.des,  tune.threshold = TRUE),
    ...
    mbo = makeTuneControlMBO(mbo.control = mbo.ctrl, mbo.design = mbo.des, tune.threshold = TRUE, learner = mbo.lrn)
  )
  for (ctrl in ctrls) {
