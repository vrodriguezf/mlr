context("Rlearner_classif_fdausc.depth")

test_that("classif_fdausc.depth behaves like original api", {
  ...
  lrn = makeLearner("classif.fdausc.depth", par.vals = list(par.depth = list("proj" = 5)))
...
test_that("resampling fdausc.depth", {
  requirePackagesOrSkip("fda.usc", default.method = "load")
  lrn = makeLearner("classif.fdausc.depth", par.vals = list(par.depth = list("proj" = 5)))
