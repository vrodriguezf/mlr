context("RLearner_classif_fdausckernel")
# FIXME: This is a copy of knn to be used for kernel, because no unittest
# for kernel exists.
test_that("classif_fdausckernel behaves like original api", {
  ...
  lrn = makeLearner("classif.fdausckernel")
...
test_that("predicttype prob for fda.usc", {
  requirePackagesOrSkip("fda.usc", default.method = "load")
  lrn = makeLearner("classif.fdausckernel",  predict.type = "prob")
...
test_that("resampling fdausckernel", {
  requirePackagesOrSkip("fda.usc", default.method = "load")
  lrn = makeLearner("classif.fdausckernel", par.vals = list(trim = 0.5), predict.type = "prob")
