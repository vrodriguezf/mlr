...
tunerMultiCritSmoofFun = function(learner, task, resampling, measures, par.set, ctrl, n.objectives, opt.path, show.info, convertx, remove.nas) {
  force(learner)
  force(task)
  force(resampling)
  force(measures)
  ...
  smoof::makeMultiObjectiveFunction(
    fn = function(x) {
      tunerFitnFun(x, learner, task, resampling, measures, par.set, ctrl, opt.path, show.info, convertx, remove.nas)
  }, par.set = ps2, has.simple.signature = FALSE, n.objectives = n.objectives , noisy = FALSE)
}
