...
#' @export
trainLearner.classif.multinom = function(.learner, .task, .weights = NULL,  ...) {
  if (is.null(.weights)) {
    f = getTaskFormula(.task)
    nnet::multinom(f, data = getTaskData(.task), ...)
  } else  {
    f = getTaskFormula(.task)
    nnet::multinom(f, data = getTaskData(.task), weights = .weights, ...)
  }
}
