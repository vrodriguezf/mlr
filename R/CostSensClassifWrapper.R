...
#' @export
trainLearner.CostSensClassifWrapper = function(.learner, .task, ...) {
  ...
  if (length(unique(newy)) == 1) {
    ...
    model = makeWrappedModel.Learner(.learner, m, getTaskDescription(.task), subset = .subset, getTaskFeatureNames(.task),
