...
#'
#' @export
makeRLearner.classif.fdauscglm = function() {
  makeRLearnerClassif(
    ...
    short.name = "fdauscglm",
    note = "model$C[[1]] is set to quote(classif.glm)"
  )
}

#' @export
trainLearner.classif.fdauscglm = function(.learner, .task, .subset, .weights = NULL, ...) {
...
#' @export
predictLearner.classif.fdauscglm = function(.learner, .model, .newdata, ...) {
  # transform the data into fda.usc:fdata class type.
