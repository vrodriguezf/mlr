...
#'
#' @export
makeRLearner.classif.fdausckernel = function() {
  makeRLearnerClassif(
    cl = "classif.fdausckernel",
    package = "fda.usc",
    ...
    short.name = "fdausckernel",
    note = "Argument draw=FALSE is used as default."
  )
}

#' @export
trainLearner.classif.fdausckernel = function(.learner, .task, .subset, .weights = NULL, trim, draw, ...) {
...
#' @export
predictLearner.classif.fdausckernel = function(.learner, .model, .newdata, ...) {
