...
trainLearner.oneclass.h2o.autoencoder = function(.learner, .task, .subset, .weights = NULL, ...) {
  ...
  h2of = h2o::as.h2o(d)
  ##% needed to explicit add autoencoder = TRUE, otherwise error 
  ##% Error in is.character(y) : argument "y" is missing, with no default 
  ##% strange thing: autoencoder is already set to TRUE in the makeParamSet above ??
