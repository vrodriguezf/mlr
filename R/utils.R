...
generateFeatureGrid = function(features, data, resample, gridsize = NULL, fmin, fmax, uniform = TRUE) {
  if (is.null(gridsize)) gridsize = nrow(data)
  sapply(features, function(feature) {
    ...
    if (resample == "none") {
      if (uniform) {
        ...
        id = sort(sample(1:nrow(data), gridsize))
        data[[feature]][id]
      }
    } else {
